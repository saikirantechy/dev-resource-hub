#!/usr/bin/env python3
"""Generate the security.ts library file with correct paths."""
import os
...
def main():
    # Correct: base is "." since basher runs inside dev-resource-hub/
    path = os.path.join(".", "lib", "security.ts")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
        f.write(CONTENT)
    size = len(CONTENT)
    print("Written " + str(size) + " bytes to lib/security.ts")
    # Verify
