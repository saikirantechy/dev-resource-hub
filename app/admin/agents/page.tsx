...
import { Search, Bot, Plus, Eye, Edit3, Trash2, Star } from "lucide-react";
import { useAdmin } from "@/context/AdminContext";
import { loadAgentsData, AgentItem } from "@/lib/admin/loaders";
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2"><Bot size={24} className="text-orange-400" /> Agent Management</h1>
          <p className="text-gray-500 text-sm mt-1">{items.length} agents</p>
        </div>
        {can("agents", "create") && (
          <button className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-orange-600 to-red-600 text-white text-xs font-bold hover:from-orange-500 hover:to-red-500 transition-all flex items-center gap-2">
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search agents..." className="w-full pl-11 pr-4 py-3 rounded-2xl bg-white/5 border border-white/10 focus:outline-none focus:border-blue-500/50 transition-all text-sm" />
      </div>
            ...
            <tbody>
              {filtered.slice(0, 50).map((item, i) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="p-4"><span className="px-2 py-0.5 rounded-full bg-orange-500/10 text-orange-400 text-[10px] font-bold">{item.category}</span></td>
                  <td className="p-4 text-sm">{item.pricing || item.isFree === false ? "Paid" : item.isFree ? "Free" : "N/A"}</td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      ...
                      {can("agents", "update") && <button className="p-1.5 rounded-lg hover:bg-blue-500/10 transition-all"><Edit3 size={14} className="text-gray-500" /></button>}
                      {can("agents", "feature") && <button className="p-1.5 rounded-lg hover:bg-amber-500/10 transition-all"><Star size={14} className="text-gray-500" /></button>}
                      {can("agents", "delete") && <button className="p-1.5 rounded-lg hover:bg-red-500/10 transition-all"><Trash2 size={14} className="text-gray-500" /></button>}
                    </div>
                  </td>
                </motion.tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={6} className="p-12 text-center text-gray-500 text-sm">No agents found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
