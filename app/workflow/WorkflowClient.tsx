...
import { 
  Zap, Plus, Play, Save, Share2, Bot, Code2, 
  Globe, Database, Layers, ArrowRight, Trash2, 
  Settings2, Sparkles, Wand2, Info
} from "lucide-react";
...
export default function WorkflowClient() {
  const [nodes, setNodes] = useState<Node[]>([
    { id: "1", type: "trigger", label: "GitHub Webhook", x: 100, y: 200, icon: Globe, color: "orange" },
    { id: "2", type: "agent", label: "Devin AI", x: 400, y: 200, icon: Bot, color: "blue" },
  ]);
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative">
        ...
        <aside className="w-full lg:w-72 bg-[#0a0a0f] border-r border-white/5 p-6 space-y-8 z-20">
          <div className="space-y-2">
            <h2 className="text-sm font-black uppercase tracking-widest text-gray-500">Node Library</h2>
            <p className="text-xs text-gray-600">Drag or click to add components to your AI workflow.</p>
          </div>
          ...
          <div className="space-y-3">
            {AVAILABLE_NODES.map((node) => (
              ...
              >
                <div className={`p-2.5 rounded-xl bg-${node.color}-500/10 border border-${node.color}-500/20 text-${node.color}-400 group-hover:scale-110 transition-transform`}>
                  <node.icon size={18} />
                </div>
                <div>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-white/5 space-y-4">
             ...
             <p className="text-[11px] text-gray-500 leading-relaxed">
               Based on your stack, we recommend adding <span className="text-white">v0.dev</span> for the UI layer.
             </p>
          </div>
        </aside>
        ...
        <main className="flex-1 relative bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-repeat opacity-95">
          ...
          <div className="absolute top-6 left-6 right-6 flex items-center justify-between z-30 pointer-events-none">
             <div className="flex items-center gap-3 p-1.5 glass rounded-2xl border border-white/10 pointer-events-auto">
                <button className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white"><Settings2 size={18} /></button>
                <div className="w-px h-4 bg-white/10" />
                <button className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white"><Layers size={18} /></button>
                <button className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white"><Info size={18} /></button>
             </div>
          ...
          {/* Node Canvas */}
          <div className="absolute inset-0 overflow-auto cursor-crosshair" ref={canvasRef}>
                  ...
                  >
                    <div className="p-5 rounded-2xl glass border border-white/10 hover:border-blue-500/40 transition-all cursor-grab active:cursor-grabbing shadow-2xl relative group">
                      <div className="flex items-start justify-between mb-4">
                        <div className={`p-2.5 rounded-xl bg-${node.color}-500/10 border border-${node.color}-500/20 text-${node.color}-400`}>
                          <node.icon size={20} />
                        </div>
                        <button 
                      ...
                      <div className="space-y-1">
                        <div className="text-xs text-gray-500 font-bold uppercase tracking-widest">{node.type}</div>
          ...
          <div className="absolute bottom-6 left-6 p-4 glass rounded-2xl border border-white/5 space-y-2 z-30">
             <div className="flex items-center gap-2 text-[10px] text-gray-500 font-bold uppercase tracking-widest">Workflow Metadata</div>
             <div className="flex gap-4">
                <div className="text-xs"><span className="text-white font-bold">{nodes.length}</span> Nodes</div>
                <div className="text-xs"><span className="text-white font-bold">{connections.length}</span> Connections</div>
        ...
        <aside className="hidden xl:flex w-80 bg-[#0a0a0f] border-l border-white/5 flex-col z-20">
           <div className="p-6 border-b border-white/5 flex items-center justify-between">
              <h2 className="text-sm font-black uppercase tracking-widest text-gray-500">Execution Log</h2>
              <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]" />
           </div>
           <div className="flex-1 p-6 font-mono text-[11px] space-y-4 overflow-y-auto text-gray-500">
              {isRunning ? (
                <div className="space-y-3">
                  <div className="text-blue-400 animate-pulse">[0.1s] Triggering GitHub Webhook...</div>
                  <div className="text-white animate-fade-in" style={{ animationDelay: '0.5s' }}>[0.8s] Initializing Devin AI agent context</div>
                  <div className="text-white animate-fade-in" style={{ animationDelay: '1.2s' }}>[1.5s] Agent connected to repository. Analyzing issue #42...</div>
                  <div className="text-emerald-400 animate-fade-in font-bold" style={{ animationDelay: '2.5s' }}>[3.0s] Workflow execution completed successfully.</div>
                </div>
              ) : (
           ...
           <div className="p-6 border-t border-white/5 space-y-3">
              <h3 className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Deploy Options</h3>
