...
function getLogClassName(log: string): string {
  if (log.startsWith("[SUCCESS]")) return "text-emerald-400";
  if (log.startsWith("[ERROR]")) return "text-red-400";
  return "text-gray-300";
}

export default function ExecutionPanel() {
  const { executionLogs, clearExecutionLogs, nodes, edges } =
    useWorkflowStore();
    ...
    <div className="h-64 border-t border-white/10 glass-dark flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-black/40">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleRun}
            className="flex items-center gap-1.5 px-3 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 transition-all text-xs font-bold"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 font-mono text-xs text-gray-300 space-y-2">
        {executionLogs.length === 0 ? (
          <div className="text-gray-600 italic">
            No execution logs yet. Click Run Simulation to start.
          </div>
        ) : (
