...
export const simulateExecution = async (nodes: AppNode[], edges: Edge[]) => {
  ...
  if (startNodes.length === 0) {
    store.addExecutionLog(
      "[ERROR] No clear starting node detected (cyclic graph without entry point).",
    );
    return;
  }

  store.addExecutionLog(
    `[INFO] Starting workflow execution... (${nodes.length} nodes)`,
  );
  ...
  while (queue.length > 0) {
    ...
    // Simulate work
    const nodeLabel =
      current.data?.label || current.type?.toUpperCase() || "Unknown Node";
